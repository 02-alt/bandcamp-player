...
/// What differs between the connected iPod and the Yoin library, matched by title + artist.
struct IPodDiff {
...
/// Two columns: albums only on the iPod (import them) and albums only in Yoin (copy them over).
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Space.s4) {
            ...
            HStack(alignment: .top, spacing: Space.s4) {
                column(title: "ONLY ON THE IPOD", subtitle: "\(diff.onlyOnIPod.count) — not in Yoin") {
                ...
                VStack(alignment: .leading, spacing: Space.s4) {
                    ...
                    if stats.mostPlayed.isEmpty {
                        ...
                        Button(action: onImport) {
                            Label("Import these plays into Yoin", systemImage: "square.and.arrow.down")
                                .font(.system(size: 12, weight: .bold)).foregroundStyle(p.accentInk)
                                .frame(maxWidth: .infinity).padding(.vertical, 9)
                                .background(Capsule().fill(p.accent))
                        }.buttonStyle(.soft)
                        Text("Adds your iPod listening to Yoin's history so it shows in your recap. Counts are read from the device and may be approximate.")
...
extension AppState {
    /// Pull the iPod's per-track play counts into Yoin's listening history (so they show in the
    ...
    func importIPodPlayCounts(_ albums: [IPodAlbum], device: IPodDevice) {
        ...
        // Join to library albums for a real albumID where the same record exists in Yoin.
