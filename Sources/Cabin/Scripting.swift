...
/// player tab) can read Cabin's now-playing state and drive transport, using the
/// same vocabulary as Music and Spotify:
///
///     tell application "Cabin"
...
extension NSApplication {
    ...
    /// `sound volume` — output volume on a 0–100 scale (Cabin stores it as 0…1).
...
@objc(CabinTrack)
final class CabinTrack: NSObject, @unchecked Sendable {   // immutable lets → safe to share
...
@objc(CabinPlayPauseCommand)
final class CabinPlayPauseCommand: NSScriptCommand {
...
@objc(CabinNextCommand)
final class CabinNextCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
...
@objc(CabinPreviousCommand)
final class CabinPreviousCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
