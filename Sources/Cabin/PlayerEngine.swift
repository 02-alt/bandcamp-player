...
private let volumeKey = "cabin.volume"
private let djModeKey = "cabin.djMode"
private let djPitchKey = "cabin.djPitch"
private let djReverbKey = "cabin.djReverb"
private let transitionModeKey = "cabin.transitionMode"
...
final class PlayerEngine: ObservableObject {
    ...
    @Published var eqEnabled: Bool = UserDefaults.standard.bool(forKey: "cabin.eqEnabled") {
        didSet { UserDefaults.standard.set(eqEnabled, forKey: "cabin.eqEnabled"); refreshDSP() }
    }
    @Published var eqPresetName: String = UserDefaults.standard.string(forKey: "cabin.eqPreset") ?? EQ.flat.name {
        didSet { UserDefaults.standard.set(eqPresetName, forKey: "cabin.eqPreset"); refreshDSP() }
    }

    // MARK: Room / vinyl DSP (muffle + tube warmth, on the same tap as the EQ)
    @Published var roomEnabled: Bool = UserDefaults.standard.bool(forKey: "cabin.roomEnabled") {
        didSet { UserDefaults.standard.set(roomEnabled, forKey: "cabin.roomEnabled"); refreshDSP() }
    }
    @Published var roomPresetName: String = UserDefaults.standard.string(forKey: "cabin.roomPreset") ?? Room.off.name {
        didSet { UserDefaults.standard.set(roomPresetName, forKey: "cabin.roomPreset"); refreshDSP() }
    }
    ...
    @Published var roomAmount: Double = PlayerEngine.loadRoomAmount() {
        didSet {
            ...
            UserDefaults.standard.set(v, forKey: "cabin.roomAmount")
    ...
    private static func loadRoomAmount() -> Double {
        UserDefaults.standard.object(forKey: "cabin.roomAmount") == nil
            ? 1 : min(1, max(0, UserDefaults.standard.double(forKey: "cabin.roomAmount")))
    }
    ...
    private static func loadCustomGains() -> [Float] {
        let saved = (UserDefaults.standard.string(forKey: "cabin.eqCustom") ?? "")
    ...
    func setEQBand(_ index: Int, _ db: Float) {
        ...
        UserDefaults.standard.set(eqCustomGains.map { String($0) }.joined(separator: ","), forKey: "cabin.eqCustom")
