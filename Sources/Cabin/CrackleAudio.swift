...
extension Bundle {
    ...
    /// calls `fatalError` when `Cabin_Cabin.bundle` isn't beside the executable — which crashed the
    ...
    static let cabinResources: Bundle? = {
        let bundleName = "Cabin_Cabin.bundle"
        let candidates = [
...
final class CrackleAudio {
    ...
    private nonisolated static func loadBundledLoop() -> AVAudioPCMBuffer? {
        ...
        guard let bundle = Bundle.cabinResources else { return nil }
