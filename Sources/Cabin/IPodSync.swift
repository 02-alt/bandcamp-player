...
extension Notification.Name {
    ...
    static let ipodDBChanged = Notification.Name("cabin.ipodDBChanged")
}
...
extension AppState {
    ...
    /// Copy tracks from the iPod into Cabin's library (import as local albums). Non-destructive.
...
enum IPodSyncEngine {
    ...
    static func run(jobs: [Job], volume: URL, dbURL: URL, serial: String) async -> String {
        ...
        try? dbData.write(to: dbURL.appendingPathExtension("cabinbak"))
    ...
    /// Copy iPod audio files into Cabin's library folder, naming each file by its REAL track title
